    ...
    <div class="col col-md-6 mb-3">
        <div class="card border-left-primary shadow h-100 ">
            <div class="card-body">
                ...
                @foreach($media as $m)
                <form class="row g-3" method="POST" action="{{ route('update.media', $m->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-6">
                        <label for="inputNomor" class="form-label">Nama Media</label>
                        <input value="{{ $m->nama_media }}" name="nama_media" type="text" class="form-control" id="inputNomor" placeholder="Nomor">
                    </div>
                    <div class="col-md-6">
                        <label for="inputTahun" class="form-label">Headline</label>
                        <input value="{{ $m->headline }}" name="headline" type="text" class="form-control" id="inputTahun" placeholder="Tahun">
                    </div>
                    <div class="col-md-6">
                        <label for="inputNomor" class="form-label">No Volume</label>
                        <input value="{{ $m->no_volume }}" name="no_volume" type="text" class="form-control" id="inputNomor" placeholder="Nomor">
                    </div>
                    <div class="col-md-6">
                        <label for="inputTahun" class="form-label">Tahun</label>
                        <input value="{{ $m->tahun }}" name="tahun" type="text" class="form-control" id="inputTahun" placeholder="Tahun">
                    </div>
                    
                    <div class="col-12">
                        <label for="exampleFormControlTextarea1" class="form-label">Keterangan</label>
                        <textarea name="keterangan" class="form-control" id="exampleFormControlTextarea1"
                            rows="3">{{ $m->keterangan }}</textarea>
                    </div>
                    <div class="col-12">
                        <label for="formFile" class="form-label">Cover</label>
                        <input name="cover" class="form-control" type="file" id="formFile">
                        @if ($m->cover)
                            <a href="{{ route('downloadCover.media', $m->cover) }}">{{ $m->cover }}</a>
                        @endif
                    </div>
                    ...
                    <div class="col-12 mb-2">
                        ...
                        @if($m->attachment)
                        <a href="{{ route('downloadDoc.media', $m->attachment) }}"></i>
                            {{ $m->attachment }}</a>
                        @endif
                    </div>
